...
namespace Regular_Expression_to_DFA.Utilities
{
    public class DFAPrinter
    {
        ...
        {            
            ...
            finita = CreateListOfItems(dfa.End);
            finita = finita.Remove(0, 1);
            finita = finita.Remove(finita.Length - 1, 1);
            stat = CreateListOfItems<string>(dfa.States);
            stat = stat.Remove(0, 1);
            stat = stat.Remove(stat.Length - 1, 1);
            alpha = CreateListOfItems<char>(dfa.Alphabet);
            alpha = alpha.Remove(0, 1);
            alpha = alpha.Remove(alpha.Length - 1, 1);
        }
